...
- Use Next.js `proxy.ts` (the Next.js 16 replacement for `middleware.ts`) for lightweight, fast-path access control (redirect unauthenticated users away from protected routes). The proxy is a UX guard only — it must not be the sole authorization check. Every protected API route independently verifies auth and role server-side.
...
- `app/` — Next.js App Router pages, layouts, loading states, and error boundaries. Route groups `(public)`, `(auth)`, and `(citizen)` enforce layout boundaries and access tiers. `app/admin/` is the admin panel. `app/api/` contains API route handlers (Cloudinary signing only — mutations use Server Actions).
- `components/` — Shared React components, organized by concern: `components/ui/` for Shadcn/ui primitives, `components/maps/` for Leaflet map components (all `"use client"`), `components/browse/` for browse page UI (filter bar, pagination, photo gallery, browse map), `components/reports/` for report-specific UI (cards, forms, photo gallery, status badge), `components/admin/` for admin-specific UI (queue table, rejection modal, action buttons, feedback note editor, analytics charts), and `components/notification-bell.tsx` for the in-app notification dropdown.
...
- `emails/` — Template string HTML generators for report status and feedback notification emails. `emails/render.ts` exports `baseLayout`, `renderApprovedEmail`, `renderRejectedEmail`, `renderResolvedEmail`, `renderFeedbackAcknowledgedEmail`, `renderFeedbackNoteAddedEmail`, and `renderFeedbackClosedEmail`. React Email was not used — `react-dom/server` cannot be imported in Next.js App Router, so emails are built via string interpolation. Auth email templates (verification, password reset) live in the Supabase dashboard — they are not maintained in this folder.
