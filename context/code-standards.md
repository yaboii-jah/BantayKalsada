...
- Transactional email for report status notifications (approved, rejected, resolved) is sent exclusively through the Resend client in `lib/email.ts`. No other file calls Resend directly. Auth emails (verification link, password reset link) are sent by Supabase Auth automatically — do not replicate them with Resend.
