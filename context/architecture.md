...
- `app/api/` — All server-side API route handlers. Owns no UI. Responsible for: report CRUD (`/api/reports`), admin moderation actions (`/api/admin/reports/[id]/approve`, `/reject`, `/resolve`), Cloudinary signed upload preset generation (`/api/uploads/sign`), and notification reads (`/api/notifications`). Every handler validates its input with Zod before touching the database. There are no NextAuth endpoints — Supabase Auth is handled outside the Next.js API layer.

- `components/` — Shared UI components not tied to a single route. Subdivided into `components/ui/` (Shadcn/ui primitives), `components/maps/` (Leaflet map components — always client-side only), `components/reports/` (report card, report form, photo upload widget, photo gallery, status badge), and `components/admin/` (admin queue table, rejection modal, action buttons).

- `lib/` — Shared utilities and Supabase client factories. Contains: `lib/supabase/server.ts` (Supabase server client factory — creates a per-request client using `@supabase/ssr`'s `createServerClient`), `lib/supabase/client.ts` (Supabase browser client — created once using `createBrowserClient`), `lib/supabase/middleware.ts` (session refresh helper called in `middleware.ts`), `lib/email.ts` (Resend client), `lib/cloudinary.ts` (Cloudinary config and signing helper), `lib/validations/` (all Zod schemas), and `lib/utils.ts` (general helpers).
...
- `emails/` — React Email templates for report status notifications only: `report-approved.tsx`, `report-rejected.tsx`, `report-resolved.tsx`. Auth email templates (verification, password reset) are managed inside the Supabase dashboard — they do not live in this folder.
...
5. **Admin authorization is verified server-side in every admin API route handler.** Middleware redirects are a UX convenience only. Every handler under `app/api/admin/` independently reads the Supabase session, fetches the user's profile, and confirms `profile.role === 'ADMIN'` before executing any logic. A missing session or non-admin role returns a 403 immediately.

6. **Rate limiting is enforced server-side.** The `POST /api/reports` handler rejects submissions when the authenticated user has already submitted 5 or more reports within the past 24 hours, counted via a database query. Client-side disabling of the submit button is a UX courtesy only and cannot be relied upon.
...
8. **Report status transitions are unidirectional and explicit.** The only valid transitions are `PENDING → APPROVED`, `PENDING → REJECTED`, and `APPROVED → RESOLVED`. No API route may accept an arbitrary status value from the request body. Each transition is a dedicated endpoint (`/api/admin/reports/[id]/approve`, `/reject`, `/resolve`) that hard-codes the target status in the server-side update — status is never a free-form client-supplied value.
