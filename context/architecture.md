...
- **Supabase PostgreSQL**: All structured application data. The `auth.users` table is Supabase-managed and stores credentials, email verification status, and session tokens — this table is never written to directly from application code. A `profiles` table, owned by the application, is kept in sync with `auth.users` via a PostgreSQL trigger that fires on new user creation; it stores the user's display name and `role` (`CITIZEN` or `ADMIN`). The `reports` table stores title, description, category, status, rejection reason, an array of Cloudinary photo URLs, latitude, longitude, a PostGIS `geography(Point, 4326)` generated column (`location`) for spatial queries, location label, and all relevant timestamps. The `report_comments` table stores user comments on approved/resolved reports with single-level threading (top-level comments and flat replies). The `notifications` table stores in-app notification records (type, read status, linked report ID or feedback ID). The `feedback` table stores app feedback submissions (type, title, description, optional rating, status, admin note — a nullable text field that admins can write and update independent of the status lifecycle). The `upload_sign_log` table stores signature request timestamps for rate limiting the Cloudinary upload endpoint. No binary file data, passwords, or auth tokens are stored in application-owned tables.
