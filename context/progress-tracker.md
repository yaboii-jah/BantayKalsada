...
Audit + remediation of admin mobile layout and admin performance. Verified: `npx tsc --noEmit` clean, `npm run lint` clean, `npm run build` passes (32 routes, `/admin` now dynamic). Manual check needed: mobile drawer open/close/backdrop/nav-tap, Part-2 pages at 360px, dashboard pre- and post-migration, bulk bar wrap.

- [x] **Sidebar → mobile overlay drawer** — `components/admin/admin-shell.tsx` rewritten: desktop `lg+` keeps the in-flow `w-64` rail with the persisted hide toggle; below `lg` the sidebar renders as a `fixed inset-y-0 left-0 z-50 w-64` drawer over a `bg-black/60` backdrop (`z-40`), with a top bar always visible on mobile (`PanelLeftOpen` hamburger) — content is never pushed. Pure CSS breakpoints + transient `mobileOpen` state (hydration-safe, no matchMedia); closes via backdrop tap, `PanelLeftClose`, or nav/sign-out click (`onNavigate`). `AdminSidebar` gained `onNavigate?: () => void` (nav `Link` `onClick` + `handleSignOut` call it). Main padding `p-4 sm:p-6`.
- [x] **Mobile CSS fixes (Part 2)** — `bulk-action-bar.tsx` stacks (`flex-col gap-3 sm:flex-row`) with wrapped buttons and `px-4 sm:px-6`; admin feedback tabs `flex-wrap`; `admin-feedback-table.tsx` `overflow-x-auto` + table `min-w-[640px]`; report review badge row `flex-wrap` + date `w-full sm:w-auto`; the 4 queue pages headers `flex flex-wrap items-center justify-between gap-3` + table cards `overflow-hidden rounded-lg border border-border bg-card`; search input `w-full ... sm:w-56`; skeleton cards `overflow-hidden`; severity pills `flex flex-wrap gap-3` (dead `resetKey` removed from `admin-report-edit-form.tsx`).
