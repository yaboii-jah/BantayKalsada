...
- [x] **React #301 on `/admin/feedback/[id]` (investigation)** — no hook-order violation found: all client components on the route (`FeedbackActions`, `FeedbackNoteEditor`, `PhotoGallery`, `AdminSidebar`, `ui/carousel`) call hooks unconditionally, `rules-of-hooks` is enforced by eslint-config-next and lint passes, and both routes serve clean (307 → login redirect) in a dev-server check. Conclusion: client-side, most consistent with the **stale PWA service-worker chunk race** (known gotcha in this repo — see earlier "Stale PWA service worker hides new map code" note). SW already has `skipWaiting` + `clientsClaim` + activate-time navigation-cache cleanup. Remediation for the user: DevTools → Application → Service Workers → **Unregister** + "Clear storage", then hard-reload; if it reappears, reproduce logged-in under `npm run dev` for the full non-minified error and share the component stack.
