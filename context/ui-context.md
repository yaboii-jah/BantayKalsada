...
- **Page background**: A very subtle dot grid (`bg-dot-grid`) is applied to the `<main>` element of all user-facing routes — `(public)` and `(citizen)` — using a `radial-gradient` with 6% black dots (24px spacing) in light mode and 5% white dots in dark mode. The pattern is barely visible, just adds gentle texture behind content without competing with cards or containers. Not applied to admin or auth layouts.
