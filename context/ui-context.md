...
- **Page background**: A very faint radial glow (`bg-radial-glow`) washes from the top-center of the `<main>` element on all user-facing routes — `(public)` and `(citizen)`. Uses a `radial-gradient(ellipse at 50% 0%, oklch(0.488 0.243 264 / 0.06) 0%, transparent 60%)` — a subtle primary-tinted highlight that adds a hint of depth without competing with content. Not applied to admin or auth layouts.
