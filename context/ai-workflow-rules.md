...
- `types/database.types.ts` — Never edit this file by hand. It is generated by the Supabase CLI (`supabase gen types typescript --local > types/database.types.ts`) and must be regenerated whenever the database schema changes. **One-off exception (2026-08-18):** `report_flag_actions` was hand-added (Supabase CLI not installed) — regenerate via `supabase gen types` once the CLI is available to reconcile parity.
