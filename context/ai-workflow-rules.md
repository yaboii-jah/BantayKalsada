...
- `types/database.types.ts` — Never edit this file by hand. It is generated by the Supabase CLI (`supabase gen types typescript --local > types/database.types.ts`) and must be regenerated whenever the database schema changes. **One-off exceptions (2026-08-18/19):** `report_flag_actions` and the `REPORT_FLAGGED_OWNER` notification_type value (migration `20260819000001`, hand-edited in both the `Enums` union and the array) were hand-added (Supabase CLI not installed) — a single `supabase gen types` regeneration reconciles parity for both once the CLI is available.
