...
export default async function AdminResolvedPage({ searchParams }: PageProps) {
    ...
    <div className="pb-20">
      <div className="mb-6 flex items-center justify-between">
        <h1 className="flex items-center gap-2 text-2xl font-bold text-foreground">
          <CheckCheck className="h-6 w-6 text-status-resolved" />
          Resolved Reports
          {totalCount !== null && (
        ...
        {totalCount !== null && totalCount > 0 && (
          <a
            href="/api/admin/export?status=RESOLVED"
      ...
      {rows.length === 0 ? (
        <div className="flex flex-col items-center gap-2 py-16 text-center">
          ...
          <p className="text-lg font-medium text-foreground">
            {hasReportFilters(filters) ? "No matching reports" : "No resolved reports"}
          </p>
        ...
        <>
          ...
          <div className="mt-4">
            <PaginationBar
              ...
              buildHref={(page) => buildAdminListHref("/admin/resolved", page, filters)}
            />
          </div>
        </>
      )}
